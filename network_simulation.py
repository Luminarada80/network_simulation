        ...
        while True:
            ...
            # Only try this network and ruleset a few times if its the first chunk
                ...
                if num_tries == 4:
                    return False
                # else:
                #     print(f'Found {num_tries} chunk(s) that works')
                #
...
def simulate_network():
    ...
    graphml_dir = '/home/emoeller/github/scBONITA2/input/custom_graphml_files'

    os.makedirs(os.path.join(script_dir, 'data'), exist_ok=True)
    os.makedirs(os.path.join(script_dir, 'data/rules'), exist_ok=True)
    os.makedirs(os.path.join(script_dir, 'data/datasets'), exist_ok=True)

    rules_filename = f"{script_dir}/data/rules/network_rules_{num_genes}_genes_{num_cells*chunks}_cells.txt"
    network_filename = f"{graphml_dir}/test_network_{num_genes}_genes_{num_cells*chunks}_cells.graphml"
    data_filename = f"{script_dir}/data/datasets/test_data_file_{num_genes}_genes_{num_cells*chunks}_cells.csv"
